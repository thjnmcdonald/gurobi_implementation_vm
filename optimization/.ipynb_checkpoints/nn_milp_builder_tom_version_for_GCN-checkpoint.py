...
def get_weights_for_GCN(state_dict, GCN_layer_included = True):
    """
...
def update_bounds_for_GCN(x_var, s_var, z_var, lb=None, ub=None):
    """
...
def nn_milp_builder_for_GCN(model: gp.Model, node, input_vars, output_vars, weights, hidden_bounds=None, c_id=0):
    """
    ...
    for i, weights_i, in enumerate(weights[:-1]):
        ...
        for j in range(nu):
            ...
            if hidden_bounds[i][j]:
                ...
                update_bounds_for_GCN(x_var, s_var, z_var, lb, ub)
                model.update()
...
def build_milp_and_run_bt_for_GCN(model: gp.Model, node, input_vars, output_vars, nn_model, c_id=0, bt_procedures=None):
    """
    ...
    # Get NN weights
    weights = get_weights_for_GCN(nn_model)

    # Tighten bounds
    input_bounds_list = []
    for k in range(n):
        input_bounds = [(v.getAttr(gp.GRB.Attr.LB), v.getAttr(gp.GRB.Attr.UB)) for v in input_vars[k]]
        input_bounds_list.append(input_bounds)
        
    output_bounds = [(v.getAttr(gp.GRB.Attr.LB), v.getAttr(gp.GRB.Attr.UB)) for v in output_vars]
    
    # we should alter this so it takes input_bounds_list
    bounds = run_bound_tightening_for_GCN(weights, input_bounds, output_bounds, bt_procedures)
    ...
    nn_milp_builder_for_GCN(model, node, input_vars, output_vars, weights, hidden_bounds, c_id)
    model.update()
    return model
