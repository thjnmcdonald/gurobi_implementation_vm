...
from GCN.optimization.bound_tightening.bt_utils import extract_bounds, build_bt_milp
...
def fbbt_linear(x_lb, x_ub, w, b, d_max):
    """
    ...
    max_norm_term = (d_max + 1)/(((2)**(0.5)) * ((d_max + 1)**(0.5)))
    ...
    m.addConstr(gpy.quicksum(max_norm_term * w[i] * x[i] for i in range(len(x))) + b, gpy.GRB.EQUAL, y)
...
def bt_lrr(weights, bounds, d_max):
    """
    ...
    for i, weights_i in enumerate(weights):
        ...
        for j in range(nu):
            x_lb, x_ub = fbbt_linear(x_prev_lb, x_prev_ub, w[:, j], b[j], d_max)
            
            # Intersect with given bounds
            if new_bounds[i + 1][j]:
                h_lb, h_ub = new_bounds[i + 1][j]
                
                # we remove the following: but change it when we get more layers. 
